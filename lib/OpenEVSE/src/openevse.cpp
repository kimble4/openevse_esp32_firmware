...
void OpenEVSEClass::setTemperature(int sensor, double temperature, std::function<void(int ret)> callback)
{
  if (!_sender) {
    return;
  }

  // SP sensor temperature
  //  $SP 3 214 - set temp3 to 21.4C

  char command[64];
  snprintf(command, sizeof(command), "$SP %u %.0f", sensor-1, temperature*10);
